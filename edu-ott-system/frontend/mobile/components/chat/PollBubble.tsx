...
export default function PollBubble({
  ...
  const handleVote = useCallback(async (idx: number) => {
    if (poll.isClosed) {
      Alert.alert('Th�ng b�o', 'B�nh ch?n n�y d� k?t th�c');
      return;
    }
    if (poll.expiredAt && new Date(poll.expiredAt) < new Date()) {
      Alert.alert('Th�ng b�o', 'B�nh ch?n n�y d� h?t h?n');
      return;
    }
    ...
    try {
      ...
      Alert.alert('L?i', err.message || 'Kh�ng th? g?i b�nh ch?n');
    } finally {
      setVoting(false);
    }
    ...
    <>
      ...
      >
        <View style={styles.header}>
          ...
          {isClosed && <Text style={styles.closedText}>Đã KẾT THÚC</Text>}
        </View>
            ...
            return (
              ...
              >
                ...
                <View style={styles.optionContent}>
                  ...
                  <Text style={[styles.optionStats, { color: isVoted ? brand : colors.muted }]}>{count} lượt {percent}%</Text>
                </View>
              </TouchableOpacity>
            );
          })}
        ...
        <View style={styles.footer}>
          <Text style={{ color: colors.muted, fontSize: 12 }}>{totalVotes} lượt bình chọn</Text>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            ...
            {!poll.isAnonymous && totalVotes > 0 ? (
              <TouchableOpacity onPress={() => setDetailVisible(true)}>
                <Text style={{ color: brand, fontSize: 12, fontWeight: '700' }}>Xem chi tiết</Text>
              </TouchableOpacity>
            ) : null}
          </View>
        </View>
          ...
          <View style={styles.modalOverlay}>
            <View style={[styles.modalCard, { backgroundColor: colors.surface }]}> 
              <View style={styles.modalHeader}>
                <Text style={{ fontSize: 17, fontWeight: '700', color: colors.text }}>Chi tiết bình chọn</Text>
