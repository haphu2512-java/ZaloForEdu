...
export default function ChatPage() {
    ...
    const fetchPinnedMessages = async (convId) => {
        try {
            const res = await conversationService.getPinnedMessages(convId);
    ...
    const jumpToMessage = (messageId) => {
        ...
        if (el) {
            ...
            toast.error("Không tìm thấy tin nhắn hoặc tin nhắn quá cũ");
        }
    };

    const jumpToPoll = (pollId) => {
        const originalMsg = messages.find(m => m.type === 'poll' && String(m.pollId?._id || m.pollId) === String(pollId));
        if (originalMsg) {
            jumpToMessage(originalMsg._id);
        } else {
            toast.error('Không tìm thấy tin nhắn bình chọn');
        }
    };
    ...
    const handlePinMessage = async (messageId) => {
        ...
        try {
            // Optimistic update (optional but good for speed)
            // setPinnedMessages(prev => [{ messageId: { _id: messageId }, ... }, ...prev]); 
    ...
    const handleUnpinMessage = async (messageId) => {
        ...
        setPinnedMessages(prev => prev.filter(p => (p.messageId?._id || p.messageId) !== messageId));
    ...
    useEffect(() => {
        ...
        const handlePinnedItemsUpdated = (payload) => {
            const { conversationId, pinnedItems } = payload;
            const currentActiveId = String(activeConvIdRef.current || '');
            if (String(conversationId) === currentActiveId) {
                setPinnedMessages(pinnedItems || []);
            }
            
            // Cập nhật lại thông tin conversation trong danh sách bên trái nếu cần
            setConversations(prev => prev.map(c => 
                String(c._id) === String(conversationId) ? { ...c, pinnedItems } : c
            ));
        };
        const handlePollUpdated = (updatedPoll) => {
            const pollId = String(updatedPoll._id);
            const convId = String(updatedPoll.conversationId?._id || updatedPoll.conversationId);
            
            // 1. Cập nhật trong luồng tin nhắn nếu đang xem đúng conversation
            if (convId === String(activeConvIdRef.current || '')) {
                setMessages(prev => prev.map(m => {
                    const mPollId = String(m.pollId?._id || m.pollId || '');
                    if (m.type === 'poll' && mPollId === pollId) {
        ...
        <div className={`chat-page ${appliedTheme === 'dark' ? 'dark-mode' : ''}`} ref={pageRef}>
                ...
                <main className="chat-main">
                    ...
                    <div className="chat-messages">
                        ...
                        {activeConversation && pinnedMessages.length > 0 && (
                            ...
                            >
                                <div style={{ color: 'var(--z-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', width: 28, height: 28, borderRadius: '50%', background: 'rgba(0,104,255,0.08)' }}>
                                    <FaThumbtack size={14} style={{ transform: 'rotate(45deg)' }} />
                                </div>
                                
                                <div style={{ flex: 1, minWidth: 0, cursor: 'pointer' }} onClick={() => jumpToMessage(pinnedMessages[0].messageId?._id || pinnedMessages[0].messageId)}>
                                    <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--z-primary)', marginBottom: 2 }}>Tin nhắn</div>
                                    <div style={{ fontSize: 13, color: 'var(--z-text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                        {pinnedMessages[0].messageId?.senderId?.fullName || pinnedMessages[0].messageId?.senderId?.username || 'Ai đó'}: {pinnedMessages[0].messageId?.content || '[Hình ảnh/File]'}
                                    </div>
                                </div>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                    {pinnedMessages.length > 1 && (
                                        <div 
                                            className="pinned-dropdown-trigger"
                                            style={{ 
                                                fontSize: 12, 
                                                fontWeight: 600, 
                                        ...
                                        >
                                            +{pinnedMessages.length - 1} ghim <FaChevronDown size={10} />
                                        </div>
                                    )}
                                    <button 
                                        title="Bỏ ghim"
                                        style={{ border: 'none', background: 'none', cursor: 'pointer', color: 'var(--z-text-muted)', padding: 4 }}
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            setUnpinTargetId(pinnedMessages[0].messageId?._id || pinnedMessages[0].messageId);
                                                ...
                                                {item.type === 'system' ? (
                                                    <div style={{ display: 'flex', justifyContent: 'center', margin: '4px 0' }}>
                                                        <span style={{ 
                                                            ...
                                                            {item.content.includes('đã ghim tin nhắn') && <FaThumbtack size={11} style={{ color: '#F59E0B', transform: 'rotate(45deg)' }} />}
                                                            {item.content.includes('bình chọn') && <FaPoll size={12} style={{ color: '#0068FF' }} />}
                                                            {item.content.includes('đã bỏ ghim') && <FaThumbtack size={11} style={{ color: '#9CA3AF' }} />}
                                                            {item.content}
                                                            {item.content.includes('đã ghim tin nhắn') && (
                                                                <span 
                                                                    style={{ color: '#0068FF', cursor: 'pointer', fontWeight: 700, marginLeft: 8 }}
                                                                    onClick={() => setShowRightPanel(true)}
                                                                >
                                                                    Xem
                                                                </span>
                                                            )}
                                                            {item.content.includes('bình chọn') && item.pollId && (
                                                                <span 
                                                                    style={{ color: '#0068FF', cursor: 'pointer', fontWeight: 700, marginLeft: 8 }}
                                                                    onClick={() => jumpToPoll(item.pollId?._id || item.pollId)}
                                                                >
                                                                    Xem
                                                                </span>
                                                            )}
                                                    ...
                                                    <div style={{ display: 'flex', justifyContent: 'center', margin: '8px 0' }}>
                                                        <span style={{ 
                                                            ...
                                                            {(item.reminderId) && (
                                                                <span 
                                                                    style={{ color: '#0068FF', cursor: 'pointer', fontWeight: 700, marginLeft: 4 }} 
                                                    ...
                                                    <MessageBubble
                                                        ...
                                                        isMe={isMe}
                                                        conversationId={activeConversation._id}
                                                        isPinned={pinnedMessages.some(p => String(p.messageId?._id || p.messageId) === String(item._id))}
            ...
            {showPinLimitModal && (
                <PinLimitModal
                    ...
                    onReplace={async (oldId) => {
                        try {
                            // Cập nhật UI ngay lập tức
                            setPinnedMessages(prev => {
                                const filtered = prev.filter(p => (p.messageId?._id || p.messageId) !== oldId);
                                // Chúng ta chưa có đủ data của tin nhắn mới nên tốt nhất là đợi socket
                                return filtered;
                            });
                            await conversationService.unpinMessage(activeConversation._id, oldId);
            ...
            {showUnpinConfirmModal && (
                <UnpinConfirmModal
                    ...
                    onConfirm={() => handleUnpinMessage(unpinTargetId)}
                />
            )}
        </div>
    );
